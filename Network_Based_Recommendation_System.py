...
for index in xrange(1, number_of_training_set_test_set_instances + 1):
	...
	print "Current time: " + str(time.asctime(time.localtime()))
	test_graph_users_items = homework_3.create_graph_set_of_users_set_of_items(test_set_file)
	...
	print "Current time: " + str(time.asctime(time.localtime()))
	training_graph_users_items = homework_3.create_graph_set_of_users_set_of_items(training_set_file)
	...
	print "Current time: " + str(time.asctime(time.localtime()))
	item_item_graph = homework_3.create_item_item_graph(training_graph_users_items)
	...
	for current_user_id in test_graph_users_items['users']:
		...
		preference_vector = homework_3.create_preference_vector_for_teleporting(current_user_id, training_graph_users_items)
		personalized_pagerank_vector_of_items = homework_3.pagerank(M, N, nodelist, alpha=0.85, personalization=preference_vector)
		sorted_list_of_recommended_items_form_PERSONAL_recommendation = homework_3.create_ranked_list_of_recommended_items(personalized_pagerank_vector_of_items, current_user_id, training_graph_users_items)
		current_dcg_for_PERSONAL_recommendation = homework_3.discounted_cumulative_gain(current_user_id, sorted_list_of_recommended_items_form_PERSONAL_recommendation, test_graph_users_items)
		
		# Minimum Discounted Cumulative Gain.
		current_minimum_dcg = homework_3.minimum_discounted_cumulative_gain(current_user_id, test_graph_users_items)
		
		# Maximum Discounted Cumulative Gain.
		current_MAXIMUM_dcg = homework_3.maximum_discounted_cumulative_gain(current_user_id, test_graph_users_items)
	...
	sum_of_AVEARGE_normalized_DCG_LOWER_BOUND                 += avg_normalized_DCG_LOWER_BOUND
	sum_of_AVEARGE_normalized_DCG_for_PERSONAL_recommendation += avg_normalized_DCG_for_PERSONAL_recommendation
	



average_normalized_DCG_LOWER_BOUND_over_all_training_set_test_set_couples                 = sum_of_AVEARGE_normalized_DCG_LOWER_BOUND/number_of_training_set_test_set_instances
...
print
print
print "  average_normalized_DCG_LOWER_BOUND_over_all_training_set_test_set_couples                 = " + str(average_normalized_DCG_LOWER_BOUND_over_all_training_set_test_set_couples)
